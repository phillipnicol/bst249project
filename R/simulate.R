...
simulateTest <- function(n,p,p1,beta1,phi.nlp,phi.normal) {
  ...
  out.bl <- Blasso(y=y,X=X,warmup=100,iters=200)
...
FullSimulate1 <- function(reps,n,p,phi.nlp,phi.normal) {
  ...
  data.LD <- cbind(data.LD, "n=100, p=50")
  data <- cbind(data, "n=50, p=100")
